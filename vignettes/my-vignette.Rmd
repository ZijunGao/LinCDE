...
```{r true density plot, fig.height=6, fig.width = 6, fig.align="center"}
...
plot(predict.LGD.D1$testLogLikelihoodHistory, main = "LGD", xlab = "n.trees", ylab = "log-likeihood", type = "l", lwd = 3, ylim = c(-1.05, -0.75))
...
```{r estimated density plot,fig.height=6, fig.width = 6, fig.align="center"}
...
```{r importance plot LGD, fig.height=6, fig.width = 6, fig.align="center"}
...
```{r modality plot,fig.height=6, fig.width = 6, fig.show='hold', out.width="50%"}
# conditional density plot 
...
model.skewness = readRDS(file = "./data/modelSkewness.rds") # training log-like: -1.08
```
...
```{r skewness plot, fig.height=6, fig.width = 6, fig.show='hold', out.width="50%"}
# conditional density plot
...
model.centering.no = readRDS(file = "./data/modelCenteringNo.rds") # training log-like: -1.39
model.centering.OLS = readRDS(file = "./data/modelCenteringOLS.rds") # training log-like: -1.07
```
...
```{r centering density plot, fig.show="hold", out.width="50%", fig.height=6, fig.width = 6}
# conditional density plot
# without centering
...
```{r without centering density plot, fig.show="hold", out.width="50%", fig.height=6, fig.width = 6}
# relative influence plot
# without centering
importance.centering.no = summary(model.centering.no, cBars = 8, main = "Without centering")
# with centering
importance.centering.OLS = summary(model.centering.OLS, cBars = 8, main = "With centering")
```
...
model.cahousing.Gaussian = readRDS(file = "./data/modelCahousingGaussian.rds") # training log-like: -5.45
...
model.cahousing.ns = readRDS(file = "./data/modelCahousingNs.rds") # training log-like: -5.25
```
...
```{r importance plot CA housing, fig.show="hold", out.width="50%", fig.height=6, fig.width = 6}
# relative influence plot
par(mar = c(5,6,2,2))
# Gaussian basis
...
```{r Gaussian plot, fig.show="hold", out.width="50%", fig.height=5, fig.width = 5}
# landmarks for visualization
...
for(i in c(7,8,9,4,5,6,1,2,3)){
  ...
  hist(y.cahousing[selectIndex], main = paste("MedInc ", round(XGrid[i,"MedInc"],2), ", ",  "AveOccup ", round(XGrid[i,"AveOccup"],2), sep = ""), xlab = "housing price",
...
Sigma = diag((apply(X.transform, 2, quantile, 0.75) - apply(X.transform, 2, quantile, 0.25))^2) # Sigma = cov(x.transform)
Sigma = importance.matrix %*% Sigma %*% importance.matrix
mtext("Gaussian basis", outer = TRUE, cex = 1.5, font = 2)
```

...
```{r, echo = FALSE, fig.height=5.3, fig.width = 7, fig.align="center"}
XProfile = matrix(runif(3^2 * d,-1,1), nrow = 3^2, ncol = d)
XProfile[,1] = rep(c(-0.6,0,0.6), 3)
XProfile[,2] = rep(c(0.6,0,-0.6), rep(3,3))
XProfile = XProfile[rep(seq(1,9), rep(100, 9)), ]
splitPointYTest = seq(quantile(y, probs = 0)-0.3, quantile(y, probs = 1)+0.3, length.out = 101)
splitMidPointYTest = (splitPointYTest[1:100] + splitPointYTest[2:101])/2
yProfile = rep(splitMidPointYTest, 9)
trueProfile = dnorm(yProfile, mean = (0.5 * XProfile[,1] + XProfile[,1] * XProfile[,2]), sd = (0.5 + 0.25 * XProfile[,2]))
# lattice plot of the true density
latticeCDE = data.frame(rep(splitMidPointYTest, 9)); colnames(latticeCDE) = "y"
latticeCDE$density = trueProfile
latticeCDE$group = rep(seq(1,9), rep(100,9))
...
lattice::xyplot(density ~ y | factor(group), data = latticeCDE, type = "l", col = c("blue"), lty = c(3), lwd = 3, ylab=list("density", cex = 1.5), xlab = list("Y", cex = 1.5), aspect = 0.75, layout = c(3,3), strip = strip, scales = list(cex = 1), main = "true conditional densities")
```
...
```{r, echo = FALSE, fig.height=5.3, fig.width = 7, fig.align="center"}
splitPointYTest = seq(quantile(y, probs = 0)-0, quantile(y, probs = 1)+0, length.out = 101)
splitMidPointYTest = (splitPointYTest[1:100] + splitPointYTest[2:101])/2

LinCDEBoostingProfile = matrix(LinCDEPredict(X = XProfile, y = yProfile, trees = example, splitPointYTest = splitPointYTest)$density, nrow = 1)
...
latticeCDE$method = factor(c(rep(c("truth", "LinCDE"), rep(100*9,2))), levels=c("truth", "LinCDE"))
latticeCDE$group = rep(rep(seq(1,9), rep(100,9)), 2)
key=list(space="top", columns = 2,
         lines=list(col=c("blue", "red"), lty=c(3,1), lwd=3),
         text=list(c("truth", "LinCDE")), cex = 1.5)
strip = lattice::strip.custom(bg="lightgrey", factor.levels = c(latex2exp::TeX('G_1: (-0.6,0.6)'),latex2exp::TeX('G_2: (0,0.6)'),latex2exp::TeX('G_3: (0.6,0.6)'),latex2exp::TeX('G_4: (-0.6,0)'),latex2exp::TeX('G_5: (0,0)'),latex2exp::TeX('G_6: (0.6,0)'),latex2exp::TeX('G_7: (-0.6,-0.6)'),latex2exp::TeX('G_8: (0,-0.6)'),latex2exp::TeX('G_9: (0.6,-0.6)')), par.strip.text=list(col="black", cex=1, font=1))

lattice::xyplot(density ~ y | factor(group), group = method, data = latticeCDE[which(latticeCDE$group %in% seq(1,9)),], type = "l", col = c("blue", "red"), lty = c(3,1), lwd = 3, ylab=list("conditional density", cex = 1.5), xlab = list("Y", cex = 1.5), key = key, aspect = 0.75, layout = c(3,3), strip = strip, scales = list(cex = 1), main = "LinCDE's estimated conditional densities")
```
...
XProfile = XProfile[rep(seq(1,9), rep(100, 9)), ]
splitPointYTest = seq(quantile(y, probs = 0)-0.1, quantile(y, probs = 1)+0.1, length.out = 101)
splitMidPointYTest = (splitPointYTest[1:100] + splitPointYTest[2:101])/2
yProfile = rep(splitMidPointYTest, 9)

LinCDEBoostingProfile = matrix(LinCDEPredict(X = XProfile, y = yProfile, trees = exampleModality, splitPointYTest = splitPointYTest)$density, nrow = 1)
trueProfile = rep(0, length(yProfile))
index = which(XProfile[,2] < 0.2)
trueProfile[index] = (0.5 * dnorm(yProfile[index], mean = 0.25 * XProfile[index,1] + 0.8, sd = 0.3) + 0.5 * dnorm(yProfile[index], mean = 0.25 * XProfile[index,1] - 0.8, sd = 0.3))
trueProfile[-index] = dnorm(yProfile[-index], mean = 0.25 * XProfile[-index,1], sd = 0.8)
...
LinCDEBoostingProfile = matrix(LinCDEPredict(X = XProfile, y = yProfile, trees = exampleSkewness, splitPointYTest = splitPointYTest)$density, nrow = 1)
trueProfile = rep(0, length(yProfile))
trueProfile = 0.5 * dnorm(yProfile, mean = 0.25 * XProfile[,1] + 0.8, sd = 0.5+0.45*XProfile[,3]) + 0.5 * dnorm(yProfile, mean = 0.25 * XProfile[,1] - 0.8, sd = 0.5-0.45*XProfile[,3])

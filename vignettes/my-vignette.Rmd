...
```{r ture density plot,fig.height=5.3, fig.width = 7, fig.align="center"}
...
```{r depth, fig.height=4.5, fig.width = 6, fig.align="center", eval = FALSE}
# validation data
set.seed(318)
nVal = 1000; d = 20
...
```{r depth (hidden), echo = FALSE, fig.height=4.5, fig.width = 6, fig.align="center"}
predictExampleD1 = readRDS(file = "./data/predictExampleD1.rds")
plot(predictExampleD1$testLogLikelihoodHistory, pch = 16, main = "depth = 1", xlab = "iteration", ylab = "log-likeihood")

predictExampleD2 = readRDS(file = "./data/predictExampleD2.rds")
plot(predictExampleD2$testLogLikelihoodHistory, pch = 16, main = "depth = 2", xlab = "iteration", ylab = "log-likeihood")
```
...
```{r prior, fig.height=4.5, fig.width = 6, fig.align="center", eval = FALSE, tidy = FALSE}
...
examplePrior = LinCDEBoosting(y = y, X = X,   
                              z = "Gaussian", depth = 2, n.trees = 50,   
                              shrinkage = 0.1, prior = priorDensity)
predictExamplePrior = LinCDEPredict(X = XVal, y = yVal, trees = examplePrior)
plot(predictExamplePrior$testLogLikelihoodHistory,   
     pch = 16, main = "depth = 2",   
     xlab = "iteration", ylab = "log-likeihood")
```
  
```{r prior (hidden), echo = FALSE, fig.height=4.5, fig.width = 6, fig.align="center"}
predictExamplePrior = readRDS(file = "./data/predictExamplePrior.rds")
plot(predictExamplePrior$testLogLikelihoodHistory,   
     pch = 16, main = "depth = 2",   
     xlab = "iteration", ylab = "log-likeihood")
```
  ...
  * Centering parameters:
    + _augmentation_: If true, a conditional mean model is fitted first, and LinCDE boosting is applied to the residuals. The centering is recommended for responses whose conditional support varies wildly. See below for an example. Default is false.
...
example = LinCDEBoosting(y = y, X = X,   
                         z = "Gaussian", depth = 3,   
                         n.trees = 300, shrinkage = 0.02)
```

```{r LinCDE boosting model (hidden), echo = FALSE}
example = readRDS(file = "./data/example.rds")
```
...
relativeImprovement =   
  (predictExample$testLogLikelihood - predictExampleNull$testLogLikelihood)/  
  (oracle - predictExampleNull$testLogLikelihood)
```
...
predictExample = readRDS(file = "./data/predictExample.rds")
predictExampleNull = readRDS(file = "./data/predictExampleNull.rds")
oracle = mean(log(exampleDensity(X = XTest, y = yTest)))
...
```{r estimated density plot,fig.height=5.3, fig.width = 7, fig.align="center"}
# conditional density plot
XProfile = matrix(runif(3^2 * d,-1,1), nrow = 3^2, ncol = d)
XProfile[,1] = rep(c(-0.6,0,0.6), 3)
XProfile[,2] = rep(c(0.6,0,-0.6), rep(3,3))
...
```{r importance plot, fig.height=4.5, fig.width = 6, fig.align="center"}
# importance score plot
barplot(example$importanceScore, names= paste("X", seq(1,d), sep = ""),   
        main = "importance score", ylab = "importance", xlab = "covariates",  
...
exampleModality = LinCDEBoosting(y = y, X = X,   
                                 depth = 3, n.trees = 200, shrinkage = 0.02)
```

```{r modality example (hidden), echo = FALSE}
exampleModality = readRDS(file = "./data/exampleModality.rds")
```
...
```{r modality plot,fig.height=5.3, fig.width = 7, fig.align="center"}
# conditional density plot
XProfile = matrix(runif(3^2 * d,-1,1), nrow = 3^2, ncol = d)
...
exampleModalityDensity = function(X, y){
  ...
  index = which(X[,2] < 0.2)
  result[index] = 0.5*dnorm(y[index], mean = 0.25*X[index,1]+0.8, sd = 0.3) +  
    0.5 * dnorm(y[index], mean = 0.25 * X[index,1] - 0.8, sd = 0.3)
...
```{r modality importance plot, fig.height=4.5, fig.width = 6, fig.align="center"}
# importance score plot
barplot(exampleModality$importanceScore, names= paste("X", seq(1,d), sep = ""),  
        main = "importance score", ylab = "importance", xlab = "covariates",  
        cex.lab = 1, cex.main = 1)
```
...
exampleSkewness = LinCDEBoosting(y = y, X = X,  
                                 depth = 3, n.trees = 200, shrinkage = 0.02)
```

```{r skewness example (hidden), echo = FALSE}
exampleSkewness = readRDS(file = "./data/exampleSkewness.rds")
```


```{r skewness plot,fig.height=5.3, fig.width = 7, fig.align="center"}
# conditional density plot
XProfile = matrix(runif(3^2 * d,-1,1), nrow = 3^2, ncol = d)
XProfile[1:(3^2-1),1] = rep(c(-0.5,0.5), 4)
XProfile[1:(3^2-1),2] = rep(rep(c(-0.5,0.5), rep(2,2)),2)
...
exampleSkewnessDensity = function(X, y){
  if(is.null(dim(X))){X = matrix(X, nrow = 1)}
  result = 0.5 * dnorm(y, mean = 0.25*X[,1]+0.8, sd = 0.5+0.45*X[,3]) +   
    0.5 * dnorm(y, mean = 0.25*X[,1]-0.8, sd = 0.5-0.45*X[,3])
  return(result)
}
...
```{r skewness importance plot, fig.height=4.5, fig.width = 6, fig.align="center"}
# importance score plot
barplot(exampleSkewness$importanceScore, names= paste("X", seq(1,d), sep = ""),  
        main = "importance score", ylab = "importance", xlab = "covariates",  
        cex.lab = 1, cex.main = 1)
```
...
# without centering
exampleCentering = LinCDEBoosting(y = y, X = X,   
                                  depth = 2, n.trees = 200, shrinkage = 0.02)
# with centering
exampleCenteringOLS = LinCDEBoosting(y = y, X = X,  
                                     depth = 2, n.trees = 200, shrinkage = 0.02,  
                                     augmentation = TRUE,   
                                     augmentationMethod = "linearRegression")
```

```{r centering model (hidden), echo = FALSE}
exampleCentering = readRDS(file = "./data/exampleCentering.rds")
exampleCenteringOLS = readRDS(file = "./data/exampleCenteringOLS.rds")
```


```{r without centering density plot, fig.height=5.3, fig.width = 7, fig.align="center"}
# conditional density plot
XProfile = matrix(runif(3^2 * d,-1,1), nrow = 3^2, ncol = d)
XProfile[1:(3^2-1),1] = rep(c(-0.5,0.5), 4)
XProfile[1:(3^2-1),2] = rep(rep(c(-0.5,0.5), rep(2,2)),2)
...
exampleCenteringDensity = function(X, y){
  if(is.null(dim(X))){X = matrix(X, nrow = 1)}
  result = rep(0, length(y))
  index = which(X[,2] < 0.2)
  result[index] = (0.5*dnorm(y[index], mean = 4*X[index,1]+0.8, sd = 0.3) +   
                     0.5*dnorm(y[index], mean = 4*X[index,1]-0.8, sd = 0.3))
  result[-index] = dnorm(y[-index], mean = 4*X[-index,1], sd = 0.8)
  return(result)
}

# without centering
densityPlot(X = XProfile, trueDensity = exampleCenteringDensity, trees = exampleCentering)
```

```{r with centering density plot,fig.height=5.3, fig.width = 7, fig.align="center"}
# with centering
yMeanOLS = cbind(1,XProfile) %*% exampleCenteringOLS$augmentationModel$coefficients
densityPlot(X = XProfile, trueDensity = exampleCenteringDensity,  
            trees = exampleCenteringOLS, yMean = yMeanOLS)
```

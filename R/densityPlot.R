...
#' @param yMean conditional mean vector. The estimated conditional densities will be shifted to center at \code{yMean}. Default is NULL (no shift).
#'
#' @export
densityPlot = function(X, minY = 0, maxY = 0, trees = NULL, trueDensity = NULL, yMean = NULL){
  # preprocess
  ...
  if(!is.null(trees)){
    ...
    splitPointYTest = seq(min(splitMidPointY) - h, max(splitMidPointY)+h, length.out = 101)} else {
      splitPointYTest = seq(minY, maxY, length.out = 101)
    }
  splitMidPointYTest = (splitPointYTest[1:100] + splitPointYTest[2:101])/2
  yProfile = rep(splitMidPointYTest, n)
  if(is.null(yMean)){yMean = rep(0, n)}
  yProfile2 = yProfile + rep(yMean, rep(100, n))
  if(!is.null(trueDensity)){
    if(!is.null(trees)){
      trueProfile = trueDensity(XProfile, yProfile2)
      LinCDEBoostingProfile = matrix(LinCDEPredict(X = XProfile, y = yProfile2, trees = trees, splitPointYTest = splitPointYTest)$density, nrow = 1)
      # lattice plot of the true density
      latticeCDE = data.frame(rep(splitMidPointYTest, n*2) + rep(rep(yMean, rep(100, n)),2)); colnames(latticeCDE) = "y"
      ...
      key=list(space="top", columns = 2,
               ...
               text=list(c("truth", "LinCDE")), cex = 1)

      strip = lattice::strip.custom(bg="lightgrey", factor.levels = paste("R", seq(1,n)), par.strip.text=list(col="black", cex=0.8, font=1))

      plot = lattice::xyplot(density ~ y | factor(group), group = latticeCDE$method, data = latticeCDE, type = "l", col = c("blue", "red"), lty = c(3,1), lwd = 3, ylab=list("conditional density", cex = 1), xlab = list("Y", cex = 1), key = key, aspect = 0.75, layout = c(3,ceiling(n/3)), strip = strip, scales = list(cex = 1), main = "LinCDE's estimated conditional densities")
    } else {
      trueProfile = trueDensity(XProfile, yProfile2)
      # lattice plot of the true density
      ...
      plot = lattice::xyplot(density ~ y | factor(group), data = latticeCDE, type = "l", col = c("blue"), lty = 3, lwd = 3, ylab=list("conditional density", cex = 1), xlab = list("Y", cex = 1), aspect = 0.75, layout = c(3,ceiling(n/3)), strip = strip, scales = list(cex = 1), main = "true conditional densities")
    }
  } else {
    LinCDEBoostingProfile = matrix(LinCDEPredict(X = XProfile, y = yProfile2, trees = trees, splitPointYTest = splitPointYTest)$density, nrow = 1)
    latticeCDE = data.frame(rep(splitMidPointYTest, n) + rep(rep(yMean, rep(100, n)),2)); colnames(latticeCDE) = "y"
    latticeCDE$density = apply(LinCDEBoostingProfile, 2, mean)
    latticeCDE$group = rep(seq(1,n), rep(100,n))

    strip = lattice::strip.custom(bg="lightgrey", factor.levels = paste("R", seq(1,n)), par.strip.text=list(col="black", cex=0.8, font=1))

    plot = lattice::xyplot(density ~ y | factor(group), data = latticeCDE, type = "l", lty = 1, lwd = 3, ylab=list("conditional density", cex = 1), xlab = list("Y", cex = 1), col = "red", aspect = 0.75, layout = c(3,ceiling(n/3)), strip = strip, scales = list(cex = 1), main = "LinCDE's estimated conditional densities")
  }
  print(plot)
}

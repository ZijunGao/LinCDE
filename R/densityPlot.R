...
densityPlot = function(X, minY = 0, maxY = 0, trees = NULL, trueDensity = NULL, yMean = NULL){
  ...
  if(!is.null(trueDensity)){
    if(!is.null(trees)){
      ...
      plot = lattice::xyplot(density ~ y | factor(group), group = method, data = latticeCDE, type = "l", col = c("blue", "red"), lty = c(3,1), lwd = 3, ylab=list("conditional density", cex = 1), xlab = list("Y", cex = 1), key = key, aspect = 0.75, layout = c(3,ceiling(n/3)), strip = strip, scales = list(cex = 1), main = "LinCDE's estimated conditional densities")
    } else {
      trueProfile = trueDensity(XProfile, yProfile2)
      # lattice plot of the true density
